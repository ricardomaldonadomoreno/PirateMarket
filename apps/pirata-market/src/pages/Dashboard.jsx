...
export default function Dashboard({ user, onProfileUpdate }) {
  ...
  const [userRealData, setUserRealData] = useState({
    real_name: '', country: '', city: '', phone: '', shop_name_for_verif: ''
  })
    ...
    try {
      const { data } = await supabase
        ...
        .select(`
          ...
          real_name, country, city, phone, shop_name_for_verif,
          identity_verified, identity_locked, allow_identity_edit,
          business_verified
        `)
        .eq('id', user.id)
        .single()
      if (data) {
        ...
        setUserRealData({
          ...
          city: data.city || '',
          phone: data.phone || '',
          shop_name_for_verif: data.shop_name_for_verif || ''
        })
      }
  ...
  const loadVerification = async () => {
    const { data } = await supabase.from('verification_requests')
      .select('*').eq('user_id', user.id).eq('app_source', 'pirata')
  ...
  const handleChangeType = async (newType) => {
    ...
    if (!confirm(`¿Cambiar tu tipo de cuenta a ${label}? Tu verificación de negocio se reseteará.`)) return
    setChangingType(true)
    try {
      const updateData = { user_type: newType }
      if (newType === 'person') {
        updateData.business_verified = false
        updateData.shop_name_for_verif = null
      }
      await supabase.from('users').update(updateData).eq('id', user.id)
      if (verificationRequest && (newType === 'shop' || newType === 'wholesale')) {
  ...
  const handleRequestModification = async () => {
    if (!modificationReason.trim()) { alert('Escribe un motivo para la solicitud'); return }
    ...
    try {
      ...
      alert('Solicitud de modificación enviada al administrador')
      setModificationReason('')
      await loadVerification()
    } catch (error) { alert('Error: ' + error.message) }
    finally { setRequestingModification(false) }
  }
  ...
  const handleIdentityFiles = (e) => {
    ...
    if (files.length !== 2) { alert('Debes seleccionar exactamente 2 fotos (frontal y dorsal)'); return }
  ...
  const handleSaveRealData = async () => {
    if (!userRealData.real_name.trim() || !userRealData.country || !userRealData.city || !userRealData.phone.trim()) {
      alert('Completa todos los campos requeridos')
      return
    }
    try {
      const updateData = {
        real_name: userRealData.real_name,
        country: userRealData.country,
        city: userRealData.city,
        phone: userRealData.phone,
      }
      if (profile?.user_type === 'shop' || profile?.user_type === 'wholesale') {
        if (!userRealData.shop_name_for_verif.trim()) { alert('Ingresa el nombre de tu tienda/bodega'); return }
        updateData.shop_name_for_verif = userRealData.shop_name_for_verif
      }
      await supabase.from('users').update(updateData).eq('id', user.id)
  ...
  const handleSubmitVerification = async () => {
    if (identityFiles.length === 0) { alert('Sube exactamente 2 fotos de identidad'); return }
    if (identityFiles.length !== 2) { alert('Debes subir exactamente 2 fotos (frontal y dorsal)'); return }
    if (!validateFiles([...identityFiles, ...businessFiles])) return
    
    const isShopOrWholesale = profile?.user_type === 'shop' || profile?.user_type === 'wholesale'
    if (isShopOrWholesale && businessFiles.length === 0) { alert('Sube documentos del negocio'); return }
    
    setUploadingDocs(true)
    try {
      const identityUrls = await uploadDocFiles(identityFiles, 'identity')
      const businessUrls = isShopOrWholesale && businessFiles.length > 0
  ...
  const isIdentityVerified = profile?.identity_verified
  const isBusinessVerified = profile?.business_verified
  const isIdentityLocked = profile?.identity_locked
  const allowIdentityEdit = profile?.allow_identity_edit
      ...
      <div className="dashboard-container">
        ...
        <div className="verification-section">
          <div className="verification-header">
            <h2>🏅 Verificación</h2>
            <select className="user-type-select" value={userType}
          ...
          <div className={`verif-layer ${isIdentityVerified ? 'layer-approved' : ''}`}>
            <div className="verif-layer-header">
              <div className="verif-layer-title">
                <span>👤</span>
                <strong>Identidad Personal (Permanente)</strong>
                {isIdentityVerified
                  ? <span className="verif-badge badge-verified">✓ Verificada</span>
                  : <span className="verif-badge badge-pending">⏳ Pendiente</span>
                }
              </div>
            </div>

            {/* Datos Reales */}
            <div className="verif-real-data">
              <div className="verif-field">
                <label>Nombre completo real *</label>
                <input type="text" className="input" placeholder="Tu nombre real completo"
                  value={userRealData.real_name}
                  onChange={e => setUserRealData(p => ({ ...p, real_name: e.target.value }))}
                  disabled={isIdentityLocked && !allowIdentityEdit} />
              </div>
              <div className="verif-field">
                <label>País *</label>
                <input type="text" className="input" placeholder="Tu país"
                  value={userRealData.country}
                  onChange={e => setUserRealData(p => ({ ...p, country: e.target.value }))}
                  disabled={isIdentityLocked && !allowIdentityEdit} />
              </div>
              <div className="verif-field">
                <label>Ciudad *</label>
                <input type="text" className="input" placeholder="Tu ciudad"
                  value={userRealData.city}
                  onChange={e => setUserRealData(p => ({ ...p, city: e.target.value }))}
                  disabled={isIdentityLocked && !allowIdentityEdit} />
              </div>
              <div className="verif-field">
                <label>Teléfono *</label>
                <input type="tel" className="input" placeholder="+591 ..."
                  value={userRealData.phone}
                  onChange={e => setUserRealData(p => ({ ...p, phone: e.target.value }))}
                  disabled={isIdentityLocked && !allowIdentityEdit} />
              </div>
              {!isIdentityLocked && (
                <button className="btn btn-primary" onClick={handleSaveRealData}>
            ...
            <div className="verif-field">
              <label>📄 CI / Pasaporte (2 fotos: frontal y dorsal) *</label>
              <p className="verif-hint">Sube exactamente 2 fotos: frontal y dorsal. Máx. 2MB por foto.</p>
              ...
              <label htmlFor="identity-input" className="btn btn-secondary verif-upload-btn"
                style={{ opacity: isIdentityLocked && !allowIdentityEdit ? 0.5 : 1 }}>
                ...
                <div className="verif-preview-grid">
                  {identityFiles.map((f, i) => (
                    <div key={i} className="verif-preview-item">
                      ...
                      <span className="verif-doc-label">{i === 0 ? 'Frontal' : 'Dorsal'}</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="verif-layer-actions">
              {!isIdentityVerified && (
                <>
                  {verifSaved && <span className="premium-saved">✓ Documentos enviados — en revisión</span>}
                  <button className="btn btn-primary"
                    onClick={handleSubmitVerification}
                    disabled={uploadingDocs || identityFiles.length !== 2 || !userRealData.real_name || !userRealData.country || !userRealData.city || !userRealData.phone}>
                    {uploadingDocs
              ...
              {isIdentityVerified && (
                <div className="verif-locked-info">
                  <p>✓ Tu identidad ha sido verificada y está congelada.</p>
                  {!allowIdentityEdit && (
                    <p className="verif-hint">Si necesitas modificar tus datos, solicita permiso al administrador.</p>
                  )}
                </div>
              )}
            </div>
            ...
            {isIdentityVerified && allowIdentityEdit && (
              <div className="verif-modification-box">
                <label>Solicitar modificación de datos:</label>
                <textarea className="input" rows={2} placeholder="Explica por qué necesitas modificar tus datos..."
                  value={modificationReason}
                  onChange={e => setModificationReason(e.target.value)} />
                <button className="btn btn-warning"
                  onClick={handleRequestModification}
                  disabled={requestingModification || !modificationReason.trim()}>
                  {requestingModification ? 'Enviando...' : 'Solicitar modificación'}
                </button>
              </div>
            )}
            ...
            {verificationRequest?.admin_note && (
              <div className="verif-admin-note">
                <strong>📌 Nota del administrador:</strong>
                <p>{verificationRequest.admin_note}</p>
              </div>
            )}
          </div>

          {/* CAPA 2 — VERIFICACIÓN DE NEGOCIO (solo para shop/wholesale) */}
          {isShopOrWholesale && (
            <div className={`verif-layer ${isBusinessVerified ? 'layer-approved' : ''}`}>
              <div className="verif-layer-header">
                <div className="verif-layer-title">
                  <span>🏪</span>
                  <strong>Verificación de Negocio</strong>
                  {isBusinessVerified
                    ? <span className="verif-badge badge-verified">✓ Verificado</span>
                    : <span className="verif-badge badge-pending">⏳ Pendiente</span>
                  }
                </div>
              </div>

              {/* Nombre de Tienda/Bodega */}
              <div className="verif-field">
                <label>Nombre de tu {userType === 'shop' ? 'tienda' : 'bodega'} *</label>
                <input type="text" className="input" placeholder={`Ej: ${userType === 'shop' ? 'Tienda Maldonado' : 'Bodega Central'}`}
                  value={userRealData.shop_name_for_verif}
                  onChange={e => setUserRealData(p => ({ ...p, shop_name_for_verif: e.target.value }))} />
              </div>
              ...
              <div className="verif-layer-actions">
                {!isBusinessVerified && (
                  <button className="btn btn-primary"
                    onClick={handleSubmitVerification}
                    disabled={uploadingDocs || businessFiles.length === 0 || !userRealData.shop_name_for_verif}>
                    {uploadingDocs
                      ? <><span className="loading" /> Enviando...</>
                      : 'Enviar documentos de negocio'}
                  </button>
                )}
                {isBusinessVerified && (
                  <div className="verif-locked-info">
                    <p>✓ Tu negocio ha sido verificado.</p>
                  </div>
                )}
              </div>
            </div>
